...
hwclocksh()
{
    ...
    # added jdmc2
    SETUP_RTC=/opt/solo/setup-rtc.sh
    SETUP_RTC_LOG=/var/log/setup-rtc.log
    verbose_log_action_msg "About to run $SETUP_RTC ..."
    if [ -r $SETUP_RTC ] ; then
	logsave $SETUP_RTC_LOG $SETUP_RTC > /dev/null || true
    else
	verbose_log_action_msg "WARNING: couldn't find script: $SETUP_RTC"
    fi
    verbose_log_action_msg "Finished running $SETUP_RTC"
    # end of changes by jdmc2
